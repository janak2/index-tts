...
class GPT2InferenceModel(GPT2PreTrainedModel):
    ...
    def forward(
        self,
        input_ids=None,  # [num_beams, target_len+1]
        past_key_values=None,
        attention_mask=None,
        token_type_ids=None,
        position_ids=None,
        ...
        assert self.cached_mel_emb is not None  # [1, target_len, 1280]
        ...
        # Create embedding
        mel_len = self.cached_mel_emb.shape[1]  # target_len
...
class UnifiedVoice(nn.Module):
    ...
    def get_conditioning(
        self,
        speech_conditioning_input,  # [1,1024,T]
        cond_mel_lengths=None,  # [1]
    ):
    ...
    def get_emo_conditioning(
        ...
        conds_mask = self.emo_cond_mask_pad(mask.squeeze(1))  # [1,(T-3)//2 +2]
        conds = self.emo_perceiver_encoder(
            speech_conditioning_input, conds_mask
        )  # [1, 1, 1024]
        return conds.squeeze(1)  # [1, 1024]
    ...
    def get_emovec(
        self,
        emo_speech_conditioning_latent,  # [1,T,1024]
        emo_cond_lengths=None,  # [1]
    ):
        emo_vec_syn_ori = self.get_emo_conditioning(
            emo_speech_conditioning_latent.transpose(1, 2), emo_cond_lengths
        )  # [1, 1024]
        emo_vec_syn = self.emovec_layer(emo_vec_syn_ori)  # [1, 1280]
        emo_vec = self.emo_layer(emo_vec_syn)  # [1, 1280]
        return emo_vec

    def merge_emovec(
        self,
        speech_conditioning_latent,  # [1,T,1024]
        emo_speech_conditioning_latent,  # [1,T,1024]
        cond_lengths,  # [1]
        emo_cond_lengths,  # [1]
        alpha=1.0,
        ...
        base_vec = self.get_emovec(
            speech_conditioning_latent, cond_lengths
        )  # [1, 1280]

        out = base_vec + alpha * (emo_vec - base_vec)  # [1, 1280]
        return out  # [1, 1280]
