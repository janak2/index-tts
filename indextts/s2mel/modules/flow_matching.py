...
class BASECFM(torch.nn.Module, ABC):
    def __init__(
        ...
        self.in_channels = args.DiT.in_channels  # 80
        ...
        if hasattr(args.DiT, "zero_prompt_speech_token"):
            self.zero_prompt_speech_token = args.DiT.zero_prompt_speech_token  # False
        else:
    ...
    def inference(
        ...
        z = (
            torch.randn([B, self.in_channels, T], device=mu.device) * temperature
        )  # [1, 80, L + T]
    ...
    def solve_euler(
        ...
        sol = []
        # apply prompt
        prompt_len = prompt.size(-1)  # T
        prompt_x = torch.zeros_like(x)  # [1, 80, L + T]
        ...
        for step in tqdm(range(1, len(t_span))):
            ...
            if inference_cfg_rate > 0:
                ...
                ) * dphi_dt - inference_cfg_rate * cfg_dphi_dt  # [1, 80, L+T]
            else:
                dphi_dt = self.estimator(x, prompt_x, x_lens, t.unsqueeze(0), style, mu)

            x = x + dt * dphi_dt  # [1, 80, L+T]
            t = t + dt
            sol.append(x)
