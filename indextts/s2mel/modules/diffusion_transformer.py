...
class DiT(torch.nn.Module):
    ...
    def forward(
        ...
        t1 = self.t_embedder(t)  # [2, 512]
        cond = cond_in_module(cond)  # [2, L + T, 512]

        x = x.transpose(1, 2)  # [2, L + T, 80]
        prompt_x = prompt_x.transpose(1, 2)  # [2, L + T, 80]

        x_in = torch.cat([x, prompt_x, cond], dim=-1)  # 80+80+512=672 [2, L+T, 672]
        ...
        x_in = self.cond_x_merge_linear(x_in)  # [2, L+T, 512]
        ...
        x_mask = (
            ...
            .to(x.device)
            .unsqueeze(1)
        )  # torch.Size([1, 1, L+T])True
        input_pos = self.input_pos[: x_in.size(1)]  # [L+T] range（0，L+T）
        x_mask_expanded = (
            x_mask[:, None, :].repeat(1, 1, x_in.size(1), 1)
            if not self.is_causal
            else None
        )  # torch.Size([1, 1, L+T, L+T]
        x_res = self.transformer(
            x_in,  # [2, L+T, 512]
            t1.unsqueeze(1),  # [2,1, 512]
            input_pos,  # [L+T]
            x_mask_expanded,  # [1, 1, L+T, L+T]
        )  # [2, L+T, 512]
        ...
        if self.long_skip_connection:  # True
            x_res = self.skip_linear(torch.cat([x_res, x], dim=-1))  # [2, L+T, 512]
        if self.final_layer_type == "wavenet":
            x = self.conv1(x_res)  # [2, L+T, 512]
            x = x.transpose(1, 2)  # [2, 512, L+T]
            t2 = self.t_embedder2(t)  # [2, 512]
            x = self.wavenet(x, x_mask, g=t2.unsqueeze(2)).transpose(
                1, 2
            ) + self.res_projection(x_res)  # long residual connection # [2, L+T, 512]
            x = self.final_layer(x, t1).transpose(1, 2)  # [2, 512, L+T]
            x = self.conv2(x)  # [2, 80, L+T]
        else:
